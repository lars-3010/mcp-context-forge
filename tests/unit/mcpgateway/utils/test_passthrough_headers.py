...
# First-Party
from mcpgateway.db import Gateway as DbGateway, GlobalConfig
...
class TestPassthroughHeaders:
    ...
    def test_basic_header_passthrough_global_config(self):
        ...
        request_headers = {
            ...
            "user-agent": "TestClient/1.0"  # Not in allowed headers
        }
    ...
    def test_gateway_specific_override(self):
        ...
        request_headers = {
            ...
            "x-tenant-id": "should-be-ignored",  # Not in gateway config
            "x-trace-id": "also-ignored"
        }
        base_headers = {"Content-Type": "application/json"}
    ...
    def test_base_header_conflict_prevention(self, caplog):
        ...
        request_headers = {
            "content-type": "text/plain",  # Conflicts with base header
            "x-tenant-id": "acme-corp"     # Should pass through
        }
        base_headers = {"Content-Type": "application/json"}
    ...
    def test_multiple_auth_type_conflicts(self, caplog):
        ...
        for auth_type in auth_types:
            ...
            if auth_type in ["basic", "bearer"]:
                ...
                assert any("Skipping Authorization header passthrough" in record.message
                          for record in caplog.records)
            else:
    ...
    def test_complex_mixed_scenario(self):
        ...
        expected = {
            "Content-Type": "application/json",
            "User-Agent": "MCPGateway/1.0",
            "X-Gateway": "gateway-value",
            "X-Conflict": "conflict-value"
        }
        assert result == expected
    ...
    def test_logging_levels(self, caplog):
        ...
        request_headers = {
            "authorization": "Bearer token",  # Will be blocked by basic auth
            "x-conflict": "request-value"     # Will conflict with base header
        }
