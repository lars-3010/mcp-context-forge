    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            if not user:
                ...
                raise HTTPException(
                    status_code=401,
                    detail="Authentication required for experimental APIs"
                )
            
            # Check experimental access
            if not has_experimental_access(user):
                logger.warning(
                    f"Unauthorized experimental API access attempt by user '{user}': "
                    f"{request.method} {request.url.path}"
                )
                raise HTTPException(
                    status_code=403,
                    detail="Experimental API access requires elevated privileges"
                )
            
            # Log successful access
            logger.info(
                f"Experimental API access granted to user '{user}': "
                f"{request.method} {request.url.path}"
            )
            ...
            response.headers["Warning"] = (
                '299 - "This is an experimental API and may change without notice"'
            )
            ...
            raise HTTPException(
                status_code=500,
                detail="Internal error processing experimental API request"
            )
